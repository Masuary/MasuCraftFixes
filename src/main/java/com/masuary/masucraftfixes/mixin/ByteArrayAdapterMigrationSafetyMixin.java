...
@Mixin(value = ByteArrayAdapter.class, remap = false)
public abstract class ByteArrayAdapterMigrationSafetyMixin {
    ...
    public final Optional<byte[]> readBits(BitBuffer buffer) {
        ...
        for (int index = 0; index < values.length; index++) {
            values[index] = (byte) adapter.readBits(buffer, null).orElse((byte) 0);
        }
